...
void Logger::msgHandler(const QtMsgType type, const QMessageLogContext& ctx, const QString& message) {
    ...
    if (defaultLogger && nonRecursiveMutex.tryLock()) {
        defaultLogger->log(type,ctx, message);
...
void Logger::log(const QtMsgType type, const QMessageLogContext&, const QString& message) {
    mutex.lock();
